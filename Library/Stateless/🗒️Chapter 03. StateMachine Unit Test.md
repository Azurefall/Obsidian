...
```ad-info
collapse: close
title: Types and Datas
~~~csharp
enum State { A, B, C, D }
enum Trigger { X, Y, Z }

const string StateA = "A", StateB = "B", StateC = "C", TriggerX = "X", TriggerY = "Y";
...
public void CanFire_GetEmptyUnmetGuardDescriptionsIfGuardPasses()
{
	...
	sm.Configure(State.A)
		.PermitIf(Trigger.X, State.B, () => true, "Guard passed");
...
## StateMachine.GetDetailedPermittedTriggers
```csharp
[Fact]
...
## StateMachine.SetTriggerParameters
```csharp
[Fact]
...
## StateConfiguration.InternalTransition
```csharp
[Fact]
...
public void WhenConfigureInternalTransitionOnTriggerWithParameters_ThenStateMachineCanFireTrigger()
{
	...
	sm.Configure(State.A)
		.InternalTransition(sm.SetTriggerParameters<string>(trigger), (arg, _) => { });
...
public void TransitionWhenBothParameterizedGuardClausesTrue()
{
	...
	sm.Configure(State.A)
		.PermitIf(x, State.B, positiveGuard, negativeGuard);
...
public void WhenConfigureConditionallyPermittedTransitionOnTriggerWithParameters_ThenStateMachineCanFireTrigger()
{
	...
	sm.Configure(State.A)
		.PermitIf(sm.SetTriggerParameters<string>(trigger), State.B, _ => true);
...
public void TransitionWhenGuardReturnsTrueOnTriggerWithMultipleParameters()
{
	...
	sm.Configure(State.A)
		.PermitIf(x, State.B, (s, i) => s == "3" && i == 3);
...
## StateConfiguration.PermitReentry
```ad-info
collapse: close
title: CountCalls
~~~csharp
...
## StateConfiguration.PermitReentryIf
```csharp
[Fact]
...
## StateMachine.OnTransitionedComplete
```csharp
[Fact]
...
## StateMachine.OnTransitioned + OnTransitionedComplete
```csharp
[Fact]
...
## StateConfiguration.OnEntry
```csharp
/// <summary>
/// with Ignore
/// State.B는 Ignore를 통해 트리거를 등록하였기 때문에 트랜지션 시 OnEntry를 무시한다.
/// </summary>
[Fact]
...
/// <summary>
/// with PermitReentry
/// State.B는 PermitReentry를 통해 트리거를 등록하였기 때문에 트랜지션 시 OnEntry를 발생시킨다.
/// </summary>
[Fact]
...
public void ExceptionWhenPermitIfHasMultipleNonExclusiveGuards()
{
	...
	sm.Configure(State.A)
		.PermitIf(x, State.B, i => i % 2 == 0)  // Is Even
		.PermitIf(x, State.C, i => i == 2);
...
public void ExceptionWhenPermitDyanmicIfHasMultipleNonExclusiveGuards()
{
	...
	sm.Configure(State.A)
		.PermitDynamicIf(x, i => i == 4 ? State.B : State.C, i => i % 2 == 0)
...
## 불가 조건(Guard) PermitReentryIf 설정 Fire
```csharp
[Fact]
