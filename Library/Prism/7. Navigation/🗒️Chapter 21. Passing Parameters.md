...
##### [PersonList.xaml] 현 모듈 내 상세정보 표시 영역인 Region("PersonDetailRegion")을 설정 - Tab control
* ListBox에 EventTrigger 설정 ("SelectionChange")
* PersonSelectedCommand 가 매핑
##### [PersonListViewModel.cs] Navigation parameter를 Navigate 요청 시 전달
...
##### [PersonDetailViewModel.cs] INavigation event 들에 Person 객체 전달
* 이벤트 발생 시 각 Person 객체에 매핑된 Tab이 존재할 경우 Tab 변경, 존재하지 않을 경우 Tab 생성
